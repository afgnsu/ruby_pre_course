class Dragon
  def initialize name
    ...
    puts "#{@name} bursts forth from an egg."
  end
  def feed
    puts "You feed #{@name}."
    @stuff_in_belly = 10
...
puts
puts 'How would you like to raise your own dragon?'
puts
...
while true
  ...
  puts "You can 'play' with your dragon, "\
        "'feed' or 'walk' your dragon, "\
        "or you can gently 'rock' or send your dragon to 'bed'."
  action = gets.chomp
  puts
  if action.downcase == 'play'
    pet.toss
  elsif action.downcase == 'feed'
    pet.feed
  elsif action.downcase == 'walk'
    pet.walk
  elsif action.downcase == 'rock'
    pet.rock
  elsif action.downcase == 'bed'
    pet.put_to_bed
  else
